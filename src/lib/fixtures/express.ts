...
export const expressModel: RepositoryModel = {
  ...
  concepts: [
    ...
    { id: "request-response", title: "Augmented req and res", summary: "Express extends Node's req/res via prototype objects, adding helpers like res.send and req.params without wrapping them.", level: 4, weight: 2, goals: ["use", "architecture", "contribute"], prerequisites: ["middleware-pipeline"] },
    ...
    { id: "error-handling", title: "The error-handling flow", summary: "next(err) skips ordinary layers and finds 4-argument error handlers; arity is the routing signal.", level: 4, weight: 3, goals: ["use", "architecture", "contribute"], prerequisites: ["middleware-pipeline"] },
    { id: "http-boundary", title: "The boundary with Node's http server", summary: "app is just a request listener; app.listen is a one-line convenience around http.createServer(app).", level: 4, weight: 2, goals: ["understand", "use", "architecture", "contribute"], prerequisites: [] },
    { id: "route-matching", title: "Path matching and params", summary: "Paths compile to regexes via path-to-regexp; named segments become req.params captures.", level: 4, weight: 2, goals: ["use", "architecture", "contribute"], prerequisites: ["routing-layer"] },
    { id: "vs-alternatives", title: "Design tradeoffs vs Koa and Fastify", summary: "Express chose callback middleware and mutable req/res; Koa chose async composition, Fastify chose schema-driven speed.", level: 5, weight: 2, goals: ["understand", "architecture", "contribute"], prerequisites: ["routing-layer", "error-handling"] },
  ],
};
